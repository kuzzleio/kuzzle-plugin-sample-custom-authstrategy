...
class AuthenticationPlugin {
  ...
  async verify (request, ssoid) {
    ...
    const response = await this.context.accessors.sdk.query(sdkRequest);
    return {kuid: response.result._id};
   ...
   * @returns {Promise<boolean>}
   */
  async validate (request, credentials, kuid, strategy, isUpdate) { // eslint-disable-line
    if (credentials.kuid) {
      throw new this.context.errors.BadRequestError('kuid cannot be specified in credentials.');
    }

    if (!credentials.ssoid) {
      throw new this.context.errors.BadRequestError('The field "ssoid" is mandatory for authentication strategy "MyCustomStrategy".');
    }

    const ssoUser = await this._get(credentials.ssoid);
    if (ssoUser === null) {
      return true;
    }

    if (kuid !== ssoUser.kuid) {
      throw new this.context.errors.PreconditionError(`Login "${credentials.ssoid}" is already used.`);
    }
